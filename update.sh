...
update_systemd_service() {
    ...
    local EXTRA_RW_PATHS=""
    if [ -f "$INSTALL_DIR/config.yaml" ]; then
        # Kerää kaikki banlist_path-arvot ja muunna hakemistopoluiksi
        local BANLIST_DIRS=$(grep "banlist_path:" "$INSTALL_DIR/config.yaml" | awk -F': ' '{print $2}' | tr -d '"' | tr -d "'" | tr -d '\r' | xargs -I{} dirname {} | sort -u)
